								...
								<div class="col-xs-12 col-md-12">		
											...
											<div role="tabpanel" class="tab-pane fade in  active" id="display-1-2">
												<div class="row">
													...
													<HEADER> 
					  <?php
require_once dirname(__FILE__).'/cot.php';

$counter = mysql_query('SELECT COUNT(`id`) FROM `tov`');
...
$page = (int) $_GET['page'];
	if ( $page > 0 && $page <= $pages ) {
		...
		$sql = "SELECT * FROM `tov` ORDER BY `date` DESC LIMIT {$start}, {$conf['pp']}";
	}
	else { 
		$sql = 'SELECT * FROM `tov` ORDER BY `date` DESC LIMIT '. $conf['pp']; 
		$page = 1;
	}
}
else {
												...
												echo"	<div class='col-md-3 col-sm-4 col-xs-12'>
														<div class='single-product'>
															...
															<div class='product-img'>
																<a href='#'>
																	<img class='primary-img'  src='{$row['image']}' alt='Product'>
																
																</a>
															</div>
															<div class='product-description'>
																<h5><a href='#'>{$row['ztext']}</a></h5>
																<div class='price-box'>
																	<span class='price'>₽{$row['otext']}</span>
																	<div>
													<div class='row'>
														<div class='col-xs-5'>
															<div class='pagination'>
																<ul>
																	<li><a href='tel:+78635470170'>Сделать заказ</a></li>
...
echo "<div class='pagination-area'>
                                                 <div class='row'>
														<div class='col-xs-5'>
															<div class='pagination'>";
if( $page > 1 ) echo'<li><a href="shop.php?page='.($page-1).'"><</a>';
if( $page < $pages ) echo '<li><a href="shop.php?page='.($page+1).'">></a>';
echo"</div>
</div>
														</div>
														<div class='col-xs-7'>
